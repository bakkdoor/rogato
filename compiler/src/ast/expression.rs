use crate::{ast::fn_call::FnCall, error::CompileError, Compile, CompileResult, Compiler};
    ...
    fn compile(&self, c: &'ctx mut Compiler<'ctx>) -> CompileResult<'ctx, FloatValue<'ctx>> {
        match self {
            ...
            Expression::OpCall(op_ident, left, right) => {
                let (c, left) = left.compile(c)?;
                let (c, right) = right.compile(c)?;
                let builder = c.context().create_builder();
            ...
            Expression::Var(id) => {
                let var = c.lookup_var(id.as_str());
                match var {
                    Some(var) => {
                        let builder = c.context().create_builder();
                        Ok((c, builder.build_load(var, id.as_str()).into_float_value()))
                    }
