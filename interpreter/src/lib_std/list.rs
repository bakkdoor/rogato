                ...
                (3, Some(a), Some(initial), Some(fn_val)) => match (&**a, &**fn_val) {
                    (Value::List(items), Value::Symbol(fn_id)) => {
                        ...
                        for item in items.reverse().iter() {
                            result =
                                match ctx.call_function(fn_id, &[result, ValueRef::clone(item)]) {
            ...
            match (args.len(), args.get(0), args.get(1)) {
                (2, Some(a), Some(fn_val)) => match (&**a, &**fn_val) {
                    (Value::List(items), Value::Symbol(fn_id)) => {
                        let mut result: Vec<ValueRef> = Vec::new();
                        for item in items.iter() {
                            match ctx.call_function(fn_id, &[ValueRef::clone(item)]) {
                                Some(val) => match &*val? {
                                    Value::List(items) => result.extend(items.iter().cloned()),
                                    _ => return error,
                                },
                                None => {
                                    return Err(NativeFnError::EvaluationFailed(
                                        fn_id.clone(),
                                        format!(
                                            "FunctionRef invalid in ^Std.List.flatMap: ^{fn_id}"
                                        ),
                                    ))
                                }
                            }
